...
public class AppOrderService {
	...
	public AppOrder completePurchase(String paymentIntentId) throws StripeException {
		...
		if (savedOrder.getItem().getSeller().getLineId() != null) {
			String message = String.format("\n 商品が購入されました！\n 商品名: %s\n 購入者: %s\n 価格: ¥%s",
					savedOrder.getItem().getName(),
					savedOrder.getBuyer().getName(),
					savedOrder.getPrice());
			// 例外は内側で処理してログ出し
			lineNotifyService.sendMessage(savedOrder.getItem().getSeller().getLineId(), message);
		}
		// 確定した注文を返す
		return savedOrder;
	}
	...
	public void markOrderAsShipped(Long orderId) {
		...
		AppOrder savedOrder = appOrderRepository.save(appOrder);
		// 買い手に LINE 通知があれば送信
		if (savedOrder.getBuyer().getLineId() != null) {
			String message = String.format("\n 購入した商品が発送されました！\n 商品名: %s\n 出品者: %s",
					savedOrder.getItem().getName(),
					savedOrder.getItem().getSeller().getName());
			// 送信試行（失敗はログのみ）
			lineNotifyService.sendMessage(savedOrder.getBuyer().getLineId(), message);
		}
	}
